-- This file is automatically generated, do not edit!
-- Item data (c) Grinding Gear Games

return {
	[1]={
		stats={
			[1]="base_secondary_skill_effect_duration"
		}
	},
	[2]={
		[1]={
		},
		stats={
			[1]="active_skill_area_of_effect_radius"
		}
	},
	[3]={
		[1]={
			[1]={
				[1]={
					k="divide_by_ten_1dp_if_required",
					v=1
				},
				limit={
					[1]={
						[1]=10,
						[2]=10
					},
					[2]={
						[1]=0,
						[2]=0
					}
				},
				rawText="{0:+d} metre to [Minion] [Aura] radius",
				text="{0:+d} metre to Minion Aura radius"
			},
			[2]={
				[1]={
					k="divide_by_ten_1dp_if_required",
					v=1
				},
				limit={
					[1]={
						[1]="#",
						[2]="#"
					},
					[2]={
						[1]=0,
						[2]=0
					}
				},
				rawText="{0:+d} metres to [Minion] [Aura] radius",
				text="{0:+d} metres to Minion Aura radius"
			},
			[3]={
				[1]={
					k="divide_by_ten_1dp_if_required",
					v=1
				},
				limit={
					[1]={
						[1]=10,
						[2]=10
					},
					[2]={
						[1]="#",
						[2]="#"
					}
				},
				rawText="[Minion] [Aura] radius is {0} metre",
				text="Minion Aura radius is {0} metre"
			},
			[4]={
				[1]={
					k="divide_by_ten_1dp_if_required",
					v=1
				},
				limit={
					[1]={
						[1]="#",
						[2]="#"
					},
					[2]={
						[1]="#",
						[2]="#"
					}
				},
				rawText="[Minion] [Aura] radius is {0} metres",
				text="Minion Aura radius is {0} metres"
			}
		},
		stats={
			[1]="active_skill_base_area_of_effect_radius",
			[2]="quality_display_active_skill_base_area_of_effect_radius_is_gem"
		}
	},
	[4]={
		[1]={
			[1]={
				[1]={
					k="milliseconds_to_seconds_2dp_if_required",
					v=1
				},
				limit={
					[1]={
						[1]=1000,
						[2]=1000
					}
				},
				rawText="Spike duration is {0} second",
				text="Spike duration is {0} second"
			},
			[2]={
				[1]={
					k="milliseconds_to_seconds_2dp_if_required",
					v=1
				},
				limit={
					[1]={
						[1]="#",
						[2]="#"
					}
				},
				rawText="Spike duration is {0} seconds",
				text="Spike duration is {0} seconds"
			}
		},
		stats={
			[1]="base_skill_effect_duration"
		}
	},
	[5]={
		[1]={
		},
		stats={
			[1]="minion_duration"
		}
	},
	[6]={
		[1]={
			[1]={
				limit={
					[1]={
						[1]=1,
						[2]="#"
					}
				},
				rawText="[Aura] grants [Minion|Minions] {0}% increased [Attack] and Cast Speed",
				text="Aura grants Minions {0}% increased Attack and Cast Speed"
			},
			[2]={
				[1]={
					k="negate",
					v=1
				},
				limit={
					[1]={
						[1]="#",
						[2]=-1
					}
				},
				rawText="[Aura] grants [Minion|Minions] {0}% reduced [Attack] and Cast Speed",
				text="Aura grants Minions {0}% reduced Attack and Cast Speed"
			}
		},
		stats={
			[1]="pain_offering_attack_and_cast_speed_+%"
		}
	},
	[7]={
		[1]={
			[1]={
				limit={
					[1]={
						[1]=1,
						[2]="#"
					}
				},
				rawText="[Aura] grants [Minion|Minions] {0}% increased damage",
				text="Aura grants Minions {0}% increased damage"
			},
			[2]={
				[1]={
					k="negate",
					v=1
				},
				limit={
					[1]={
						[1]="#",
						[2]=-1
					}
				},
				rawText="[Aura] grants [Minion|Minions] {0}% reduced damage",
				text="Aura grants Minions {0}% reduced damage"
			}
		},
		stats={
			[1]="pain_offering_damage_+%"
		}
	},
	["active_skill_area_of_effect_radius"]=2,
	["active_skill_base_area_of_effect_radius"]=3,
	["base_secondary_skill_effect_duration"]=1,
	["base_skill_effect_duration"]=4,
	["minion_duration"]=5,
	["pain_offering_attack_and_cast_speed_+%"]=6,
	["pain_offering_damage_+%"]=7,
	parent="skill_stat_descriptions",
	["quality_display_active_skill_base_area_of_effect_radius_is_gem"]=3
}